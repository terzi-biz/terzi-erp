...
export async function dashboardOverview(sb: Sb, f: DashboardFilters) {
  ...
  const qualified = (l: any) => {
    if (progressed.has(l.id)) return true;
    const stage = stageById.get(l.stage_id); const initial = stage ? initialByPipeline.get(stage.pipeline_id) : null;
    return !!stage && !!initial && n(stage.sort_order) > n(initial.sort_order) && !stage.is_lost;
  };
  const needsReview = (l: any) => {
    const stage = stageById.get(l.stage_id); const initial = stage ? initialByPipeline.get(stage.pipeline_id) : null;
    return !!stage?.is_lost && !!initial && stage.id !== initial.id && !progressed.has(l.id);
  };
  ...
  const callbackPhones = new Set(calls.filter((c) => c.direction === "outbound").map((c) => c.phone_e164 || c.phone_norm).filter(Boolean));
  const funnel = [
  ...
  const alerts = [
    ...
    missed.filter((c) => !callbackPhones.has(c.phone_e164 || c.phone_norm)).length ? { severity: "warning", title: "Пропущені без передзвону", value: missed.filter((c) => !callbackPhones.has(c.phone_e164 || c.phone_norm)).length, metric: "calls_missed", action: "Перевірити дзвінки" } : null,
  ...
  const insights = [
    ...
    missed.length ? { severity: "warning", title: `${missed.length} пропущених дзвінків`, detail: `${missed.filter((c) => callbackPhones.has(c.phone_e164 || c.phone_norm)).length} номерів мають вихідний дзвінок у вибраному періоді.`, metric: "calls_missed" } : null,
...
export async function drilldown(sb: Sb, p: DrilldownParams): Promise<DrilldownRow[]> {
  ...
  return rows.map((r: any) => ({ id: r.id, title: r.title || r.name || r.number || r.phone_e164 || r.phone_norm || "Запис", subtitle: r.status ?? r.commercial_status ?? r.direction ?? null, date: r.created_at ?? r.started_at ?? r.due_at ?? r.starts_at ?? null, amount: r.amount_total != null ? n(r.amount_total) : r.total_client != null ? n(r.total_client) : null, href: r.stage_id ? `/crm/leads?lead=${r.id}` : r.started_at ? "/crm/calls" : r.starts_at ? "/calendar" : r.commercial_status ? `/orders/${r.id}` : r.module ? "/history" : r.order_id ? `/orders/${r.order_id}` : null }));
}
...
async function dashboardOverviewFromLoaded(d: any, p: DrilldownParams) {
  ...
  const isQualified = (l: any) => { const s = stageById.get(l.stage_id); const i = s ? initial.get(s.pipeline_id) : null; return progressed.has(l.id) || (!!s && !!i && n(s.sort_order) > n(i.sort_order) && !s.is_lost); };
