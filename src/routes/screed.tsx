  ...
  head: () => ({
    meta: [
      { title: "Стяжка TERZI — калькулятор" },
      {
        name: "description",
        content:
          "Калькулятор напівсухої стяжки TERZI з цементом, кімнатами, ліфтом, логістикою і КП.",
      },
    ...
    <label className="block">
      <span className="inline-flex items-center gap-1 text-xs uppercase tracking-wider text-muted-foreground">
        ...
        {hint && (
          <span
            title={hint}
            className="cursor-help text-muted-foreground/60 hover:text-primary transition-colors"
          >
...
function ToggleSwitch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      <span
        className={`inline-block h-5 w-5 transform rounded-full bg-background shadow transition-transform ${checked ? "translate-x-5" : "translate-x-0.5"}`}
      />
    </button>
  );
}
...
const defaultInput: ScreedInput = {
  ...
  withCuts: true,
  sandType: "standard",
  withComplexPrep: false,
  withCementUnload: false,
  withDemolition: false,
  insulationType: "none",
  cityDelivery: true,
  outOfCityKm: 0,
  withLift: false,
...
function ScreedPage() {
  ...
  const { materialPrices, workPrices, logisticsPrices, priceSources, priceBookVersion } =
    useModulePricing("screed", input.area);
  ...
  const prod = useMemo(
    () => calculateScreedProduction(prodInput, prodCfg, grades),
    [prodInput, prodCfg, grades],
  );
  const comparison = useMemo(
    () => (showCompare ? compareGrades(prodInput, prodCfg, grades) : []),
    [showCompare, prodInput, prodCfg, grades],
  );
  ...
  const onPdf = () => {
    ...
    toast.info(
      "Відкрито аркуш «Кошторис / КП» — натисніть «Друк PDF» під кошторисом (з усіма правками)",
    );
  };

  const inp =
    "w-full bg-input border border-border rounded-md px-3 py-2.5 text-sm focus:border-primary hover:border-border/80 outline-none transition-colors";
  const inpWithIcon = inp + " pl-9";
  const sel =
    "w-full min-w-0 bg-input border border-border rounded-md px-3 py-2.5 text-sm focus:border-primary hover:border-border/80 outline-none transition-colors appearance-none cursor-pointer";
  const btnBase =
    "px-3.5 py-2 rounded-md text-xs font-semibold inline-flex items-center gap-2 transition-all duration-150 active:scale-[0.97]";
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-5 sm:space-y-7">
      ...
      {view === "estimate" && (
        <EstimateView
          ...
          module="Стяжка"
          area={input.area}
          thicknessCm={result.thicknessUsed}
      ...
      <div
        className="grid lg:grid-cols-[1fr_420px] gap-7"
        style={{ display: view === "calc" ? undefined : "none" }}
      >
        <div className="space-y-7">
          {/* Client */}
          <section className="panel p-6">
            <h2 className="font-bold text-sm uppercase tracking-wider mb-5 text-primary">
              {t("clientData")}
            </h2>
            <div className="mb-4">
              <EstimateLinkPicker
          ...
          <section className="panel p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <Field label="Периметр, пог.м">
                <NumberInput
                  className={inp}
                  value={input.perimeter ?? 0}
              ...
              <Field label="Кімнат" hint="Якщо більше 1 — додаються деформаційні шви / нарізка.">
                <NumberInput
                  ...
                  onChange={(v) => upd("roomsCount", v)}
                />
              </Field>
              <Field
                label="Поверх"
                hint="Поверх подачі суміші. Від 6-го поверху додається коефіцієнт підйому 5–50%."
              >
                <NumberInput
                  className={inp}
                  value={input.floor}
                  onChange={(v) => upd("floor", v)}
                />
              </Field>
              <Field
                label="Ліфт"
                hint="Наявність ліфта на замовленні — впливає на швидкість подачі матеріалу."
              >
                <ToggleSwitch checked={input.withLift} onChange={(v) => upd("withLift", v)} />
              </Field>
              <Field
                label="Товщина, см"
                hint="Робочий діапазон 4–25 см. Понад 7 см додається окрема позиція «Влаштування стяжки понад 7 см» — 15 грн/м² за кожен см."
              >
                <NumberInput
                  step="0.5"
                  className={inp}
              ...
              <Field label="Марка стяжки" hint={SCREED_GRADE_DISCLAIMER}>
                <select
                  className={sel}
                  value={input.screedGrade ?? "M200"}
                  onChange={(e) => upd("screedGrade", e.target.value as ScreedGrade)}
                >
                  {SCREED_GRADE_LIST.map((g) => (
                    <option key={g} value={g}>
                      {GRADE_LABEL[g]} · ≈{grades[g].strengthMPa} МПа
                    </option>
                  ))}
                </select>
              </Field>
              <Field
                label="Цемент"
                hint="М500 (рекомендовано) — стандарт TERZI. Auto бере цемент з профілю суміші."
              >
                <select
                  className={sel}
                  value={input.cementType}
                  onChange={(e) => upd("cementType", e.target.value as CementType)}
                >
              ...
              <Field
                label="Профіль суміші"
                hint="Економ — М400; Стандарт/Посилений — М500 з різною кількістю фібри."
              >
                <select
                  className={sel}
                  value={input.profile}
                  onChange={(e) => upd("profile", e.target.value as Profile)}
                >
              ...
              <Field
                label="Пісок"
                hint="Пісок з відсівом застосовується для посиленої стяжки (закупка 750 грн/т, продаж 850 грн/т)."
              >
                <select
                  className={sel}
                  value={input.sandType ?? "standard"}
                  onChange={(e) => upd("sandType", e.target.value as SandType)}
                >
          ...
          <section className="panel p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <Field
                label="Армувальна сітка"
                hint="Композит — легка скло/базальт-пластикова сітка (+10% площі). Метал — зварна сталева (+15% площі)."
              >
                <select
                  className={sel}
                  value={input.meshType}
                  onChange={(e) => upd("meshType", e.target.value as MeshType)}
                >
              ...
              <Field label="Утеплення" hint="Утеплювач під стяжку (ЕПС / ХПС різної товщини).">
                <select
                  ...
                  onChange={(e) => upd("insulationType", e.target.value as InsulationType)}
                >
              ...
              <OptionToggle
                label="Демпферна стрічка"
                checked={input.withDamper}
                onChange={(v) => upd("withDamper", v)}
              />
              <OptionToggle
                label="Плівка п/е"
                checked={input.withFilm}
                onChange={(v) => upd("withFilm", v)}
              />
              <OptionToggle
                label="Шліфовка"
                checked={input.withGrind}
                onChange={(v) => upd("withGrind", v)}
              />
              <OptionToggle
                label="Нарізання деформаційних швів"
                checked={input.withCuts !== false}
                onChange={(v) => upd("withCuts", v)}
              />
              <OptionToggle
                label="Розухилення"
                checked={input.withSlope}
                onChange={(v) => upd("withSlope", v)}
              />
              <OptionToggle
                label="Складна підготовка"
                checked={input.withComplexPrep}
                onChange={(v) => upd("withComplexPrep", v)}
              />
              <OptionToggle
                label="Вивантаження мішків цементу"
                checked={input.withCementUnload === true}
                onChange={(v) => upd("withCementUnload", v)}
              />
              <OptionToggle
                label="Демонтажні роботи"
                checked={input.withDemolition}
                onChange={(v) => upd("withDemolition", v)}
              />
              <OptionToggle
                label="Підйом матеріалу на поверх"
                checked={input.withLift}
                onChange={(v) => upd("withLift", v)}
              />
            </div>
          </section>
          ...
          <section className="panel p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label={t("cementDelivery")}>
                <select
                  className={sel}
                  value={input.cementDelivery}
                  onChange={(e) => upd("cementDelivery", e.target.value as CementDelivery)}
                >
              ...
              <Field label={t("sandDelivery")}>
                <select
                  className={sel}
                  value={input.sandDelivery}
                  onChange={(e) => upd("sandDelivery", e.target.value as SandDelivery)}
                >
          ...
          <section className="panel p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <label className="flex items-center gap-2 text-sm mt-7 cursor-pointer hover:text-primary transition-colors">
                <input
                  type="checkbox"
                  className="accent-primary"
              ...
              <Field label={t("partnerCommission")}>
                <NumberInput
                  className={inp}
          ...
          <section className="panel p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-5">
              <Stat
                label="Об'єм суміші"
                value={`${formatNum(prod.screedVolumeM3, 2)} м³`}
                highlight
              />
              <Stat
                label="Марка"
                value={`${GRADE_LABEL[prod.screedGrade]} · ≈${prod.strengthMPa} МПа`}
              />
              <Stat label="Середній шар" value={`${formatNum(prod.thicknessCm, 1)} см`} />
              <Stat
                label="Периметр"
                value={`${formatNum(prod.perimeterM, 1)} м.п.`}
                warn={prod.perimeterM <= 0}
              />
            </div>
            ...
            {prod.warnings.map((w) => (
              <div
                key={w}
                className="mb-4 flex items-start gap-2 p-2.5 rounded bg-warning/10 text-warning text-xs"
              >
            ...
            <ProdTable
              title="Матеріали (закупка)"
              rows={prod.materialRows}
              total={prod.materialsTotal}
            />
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-5">
              ...
              <Stat
                label="Собівартість 1 м²"
                value={`${formatNum(prod.productionCostPerM2, 0)} грн/м²`}
                highlight
              />
            </div>
            ...
            <div className="mt-5 border-t border-border pt-5">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
                <Field
                  label="Цільова маржа, %"
                  hint="Маржа рахується від виручки: Ціна = Собівартість / (1 − маржа/100)."
                >
                ...
                <Stat
                  label="Ціна клієнту / м²"
                  value={`${formatNum(prod.sellingPricePerM2, 0)} грн/м²`}
                />
              ...
              <div className="mt-3 flex gap-2">
                {[25, 30, 35, 40].map((m) => (
                  <button
                    ...
                    className={`${btnBase} ${targetMargin === m ? "bg-primary text-primary-foreground" : "bg-secondary hover:bg-secondary/80"}`}
                  >
                    {m}%
                  </button>
                ))}
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {comparison.map((c) => (
                      <tr
                        key={c.grade}
                        className={`border-b border-border/40 ${c.grade === prod.screedGrade ? "bg-primary/10" : ""}`}
                      >
                        ...
                        <td className="text-right tabular-nums">
                          {formatNum(c.plasticizerLiters, 2)} л
                        </td>
                        <td className="text-right tabular-nums">{formatUah(c.productionCost)}</td>
                        <td className="text-right tabular-nums font-semibold">
                          {formatNum(c.costPerM2, 0)}
                        </td>
                        <td className="text-right tabular-nums">
                          {c.deltaPerM2 ? `+${formatNum(c.deltaPerM2, 0)} грн/м²` : "—"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <section className="panel p-6">
              ...
              {showInternal && (
                <>
                  ...
                  <Stat
                    ...
                    highlight={result.marginPercent >= settings.marginThreshold}
                    warn={result.marginPercent < settings.marginThreshold}
                  />
                </>
              )}
            </div>
            {showInternal && result.marginPercent < settings.marginThreshold && (
              <div className="mt-4 p-4 rounded-md bg-destructive/15 border border-destructive/40 text-destructive">
                ...
                <div className="text-xs mt-1 opacity-90">
                  Поточна {formatNum(result.marginPercent, 1)}% &lt; {settings.marginThreshold}%.
                  Перегляньте ціну, знижку або обсяг.
                </div>
              </div>
            )}
            {result.warnings.length > 0 && (
              <div className="mt-4 space-y-2">
                {result.warnings
                  .filter((w) => w !== "warnLowMargin")
                  .map((w, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-2 p-2.5 rounded bg-warning/10 text-warning text-xs"
                    >
                      <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                      {t(w)}
                    </div>
            ...
            <section className="panel p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-bold text-xs uppercase tracking-wider">
                  Контрольний сценарій 100м²/7см
                </span>
              </div>
              <ul className="text-[11px] space-y-0.5 font-mono">
                {selfTest.report.map((r, i) => (
                  <li key={i} className={r.startsWith("✓") ? "text-success" : "text-destructive"}>
                    {r}
                  </li>
                ))}
              </ul>
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border pt-5">
        ...
        <EstimateDraftControls
          ...
          buttonClass={`${btnBase} bg-secondary hover:bg-secondary/80`}
        />
        <button
          onClick={onPdf}
          className={`${btnBase} bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm hover:shadow-md`}
        >
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
          ...
          <div className="shrink-0 text-right">
            ...
            <div className="text-sm font-bold tabular-nums">
              {formatNum(result.pricePerM2, 0)} грн/м²
            </div>
          </div>
        </div>
      </div>
...
function ProdTable({
  ...
  title: string;
  rows: { key: string; name: string; unit: string; qty: number; price: number; sum: number }[];
  total: number;
}) {
  return (
    <div className="mb-5">
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          ...
          <tr>
            <td
              colSpan={3}
              className="pt-2 text-right font-bold uppercase text-[10px] tracking-wider"
            >
              Разом
            </td>
...
function Stat({
  ...
  return (
    <div
      className={`p-3 rounded-md transition-colors ${warn ? "bg-destructive/10 border border-destructive/30" : highlight ? "bg-primary/10 border border-primary/30" : "bg-secondary/40 border border-transparent"}`}
    >
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
      <div
        className={`font-bold mt-0.5 ${warn ? "text-destructive" : highlight ? "text-primary" : ""}`}
      >
        {value}
      </div>
    </div>
