...
function InsulationPage() {
  ...
  const draft = useEstimateDraft<InsulationInput>({
    module: "insulation", defaultInput, defaultManager: profile?.display_name ?? "",
  });
  const { input, setInput, client, setClient, link, setLink, estimateNumber, estimateId } = draft;
  const savedStatus = draft.status;
  const { materialPrices, workPrices, priceSources, priceBookVersion } = useModulePricing("insulation", input.area);
  ...
  const result = useMemo(
    () => calculateInsulation(input, materialPrices, worksMapped, DEFAULT_INSULATION_LOGISTICS, insulationCoeffs),
    [input, materialPrices, worksMapped, insulationCoeffs],
  );

  const upd = <K extends keyof InsulationInput>(k: K, v: InsulationInput[K]) => setInput((s) => ({ ...s, [k]: v }));
  ...
  const onSaveDraft = useCallback(async () => {
    const row = await saveFn({ data: {
      ...
      calculation_json: buildEstimateSnapshot({
        module: "insulation", engineVersion: ENGINE_VERSIONS.insulation, priceBookVersion,
        inputs: input, result,
        prices: { materials: materialPrices, works: worksMapped, logistics: DEFAULT_INSULATION_LOGISTICS },
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 relative">
      ...
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 border-b border-border pb-4 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Фасад / покрівля / підлога / полістиролбетон</p>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-6 relative z-10" style={{ display: view === "calc" ? undefined : "none" }}>
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <section className="panel p-4 md:p-5">
            <h2 className="font-bold text-sm uppercase tracking-wider mb-4 text-primary">Зона утеплення</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
              ...
              ] as { id: InsZone; label: string }[]).map((z) => (
          ...
          <section className="panel p-4 md:p-5">
            <h2 className="font-bold text-sm uppercase tracking-wider mb-4 text-primary">Матеріал</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              ] as { id: InsMaterial; label: string }[]).map((m) => (
                <button key={m.id} onClick={() => upd("material", m.id)}
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <Field label="Шари"><NumberInput min="1" disabled={input.material === "polystyrcrete"} className={inp} value={input.layersCount} onChange={(v) => upd("layersCount", v)} /></Field>
            </div>
          </section>
          ...
          {input.material !== "polystyrcrete" && (
            <section className="panel p-4 md:p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={input.withGlue} onChange={(e) => upd("withGlue", e.target.checked)} />Клей</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={input.withDowels} onChange={(e) => upd("withDowels", e.target.checked)} />Дюбель-парасолька</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={input.withMesh} onChange={(e) => upd("withMesh", e.target.checked)} />Склосітка + армування</label>
              </div>
            </section>
          )}
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Field label="Контейнери на вивіз (8 м³)"><NumberInput className={inp} value={input.haulContainers} onChange={(v) => upd("haulContainers", v)} /></Field>
            </div>
          </section>
            ...
            <section className="panel p-4 md:p-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                {showInternal && (<>
                  ...
                  <Stat label="Маржа" value={`${formatNum(result.marginPercent, 1)} %`} highlight={result.marginPercent >= insulationCoeffs.marginThreshold} warn={result.marginPercent < insulationCoeffs.marginThreshold} />
                </>)}
              </div>
