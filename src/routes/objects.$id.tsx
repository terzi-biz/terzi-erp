...
import {
  getObject, deleteObject, updateObjectStatus, saveObjectZone, deleteObjectZone,
  addObjectComment, saveObjectMeasurement,
  COMMERCIAL_STATUSES, PRODUCTION_STATUSES, FINANCIAL_STATUSES, RISK_LEVELS,
  COMMERCIAL_LABELS, PRODUCTION_LABELS, FINANCIAL_LABELS, SERVICE_LABELS, OBJECT_SERVICES,
...
export const Route = createFileRoute("/objects/$id")({
  ...
  head: () => ({ meta: [{ title: "Карточка об'єкта — TERZI" }] }),
...
function ObjectDetailPage() {
  ...
  const getFn = useServerFn(getObject);
  const delFn = useServerFn(deleteObject);
  const setStatusFn = useServerFn(updateObjectStatus);
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-[1400px] mx-auto space-y-4">
        <div className="flex items-center gap-2 text-sm">
          <Link to="/objects" className="text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
        ...
        <div className="bg-card border border-border rounded-lg p-4 md:p-5 space-y-3">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            ...
            <button
              onClick={() => { if (confirm("Видалити об'єкт?")) delFn({ data: { id } }).then(() => navigate({ to: "/objects" })); }}
...
function ZonesTab({ o }: { o: any }) {
  const qc = useQueryClient();
  const saveFn = useServerFn(saveObjectZone);
  const delFn = useServerFn(deleteObjectZone);
  ...
  const save = async () => {
    ...
    await saveFn({ data: {
      object_id: o.id, name: form.name, service: form.service || null,
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground">Зони об'єкта ({o.zones?.length ?? 0})</div>
        ...
        <div className="border border-border rounded p-3 space-y-2 bg-secondary/20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <select value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })}
              ...
              {OBJECT_SERVICES.map((s) => <option key={s} value={s}>{SERVICE_LABELS[s]}</option>)}
            </select>
...
function EstimatesTab({ o }: { o: any }) {
  return (
    <div className="space-y-2">
      <div className="text-xs text-muted-foreground">Всі кошториси з полем object_id = {o.number}. Щоб прив'язати новий — при збереженні з калькулятора вкажіть об'єкт.</div>
      <div className="scroll-x">
        <table className="w-full text-sm min-w-[600px]">
...
function ProductionTab({ o }: { o: any }) {
  return (
    <div className="space-y-2">
      <div className="text-xs text-muted-foreground">Бронювання бригад для цього об'єкта. Планування — в розділі «Операційний календар».</div>
...
function CommentsTab({ o }: { o: any }) {
  const qc = useQueryClient();
  const addFn = useServerFn(addObjectComment);
  const [text, setText] = useState("");
  const send = async () => {
    if (!text.trim()) return;
    await addFn({ data: { object_id: o.id, body: text } });
