...
export const Route = createFileRoute("/roofing_pvc")({
  ...
  head: () => ({ meta: [
    ...
    { name: "description", content: "Калькулятор ПВХ-мембрани Sika: нахльост 1.15, вертикальна площа парапету, воронки та аератори d75/110/160, профілі й розхідники." },
    { property: "og:title", content: "ПВХ мембрана TERZI — калькулятор покрівлі" },
    { property: "og:description", content: "Розрахунок ПВХ-мембрани TERZI: горизонтальна і вертикальна площа, комплектація, логістика, КП." },
...
const defaultInput: PvcInput = {
  ...
  thickness: "1.5",
  withGeotextile: true, withDemount: false, withSlope: false, withPrep: true,
...
function PvcPage() {
  ...
  const { materialPrices, workPrices, workCostPrices, priceSources, priceBookVersion } = useModulePricing("roofing_pvc", input.area);
  ...
  const result = useMemo(
    () => calculatePvc(input, materialPrices, worksMapped, workCostPrices, DEFAULT_PVC_LOGISTICS, DEFAULT_PVC_COEFFS),
    [input, materialPrices, worksMapped, workCostPrices],
  );

  const upd = <K extends keyof PvcInput>(k: K, v: PvcInput[K]) => setInput((s) => ({ ...s, [k]: v }));
  ...
  const onSaveDraft = useCallback(async () => {
    const row = await saveFn({ data: {
      ...
      calculation_json: buildEstimateSnapshot({
        module: "roofing_pvc", engineVersion: ENGINE_VERSIONS.roofing, priceBookVersion,
        inputs: input, result,
        prices: { materials: materialPrices, works: worksMapped, workCosts: workCostPrices, logistics: DEFAULT_PVC_LOGISTICS },
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 relative">
      ...
      {view === "estimate" && (
        <div className="relative z-10">
          <EstimateView result={result} client={client} branding={branding}
            ...
            area={input.area} estimateNumber={estimateNumber} isInternal={isInternal} exportBlockReason={priceBlock} estimateId={estimateId}
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-6 relative z-10" style={{ display: view === "calc" ? undefined : "none" }}>
        <div className="space-y-4 md:space-y-6">
          ...
          <section className="panel p-4 md:p-5">
            <h2 className="font-bold text-sm uppercase tracking-wider mb-4 text-primary">Мембрана</h2>
            <Field label="Товщина мембрани Sika" hint="1.5 мм — стандарт житлових/адміністративних дахів. 1.8 мм — промислові та експлуатовані покриття.">
              <div className="flex gap-2">
                {(["1.5", "1.8"] as PvcThickness[]).map((t) => (
                  <button key={t} onClick={() => upd("thickness", t)}
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <Field label="Периметр парапету, п.м" hint="Сумарна довжина парапету — база для вертикальної площі.">
                <NumberInput className={inp} value={input.perimeter} onChange={(v) => upd("perimeter", v)} />
              </Field>
              <Field label="Висота парапету, м">
                <NumberInput step="0.01" className={inp} value={input.parapetHeightM} onChange={(v) => upd("parapetHeightM", v)} />
              </Field>
              <Field label="Ширина парапету, м" hint="Товщина парапету зверху — мембрана заводиться по ній.">
                <NumberInput step="0.01" className={inp} value={input.parapetWidthM} onChange={(v) => upd("parapetWidthM", v)} />
              </Field>
              <Field label="Нахльост за парапет, м" hint="Заведення мембрани за зовнішній край парапету.">
                <NumberInput step="0.01" className={inp} value={input.parapetOverlapM} onChange={(v) => upd("parapetOverlapM", v)} />
              </Field>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3 text-xs">
              ...
              <Stat label="Мембрани з нахльостом" value={`${formatNum(result.membraneM2, 0)} м²`} highlight />
            </div>
            <Tip>
              Вертикальна висота = висота + ширина парапету + нахльост. Вертикальна площа = периметр × ця висота.
            </Tip>
          </section>

          <section className="panel p-4 md:p-5">
            <h2 className="font-bold text-sm uppercase tracking-wider mb-4 text-primary">Комплектація</h2>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">Воронки, шт</div>
            <div className="grid grid-cols-3 gap-3">
              <Field label="d 75 мм"><NumberInput min={0} className={inp} value={input.funnels75} onChange={(v) => upd("funnels75", v)} /></Field>
              <Field label="d 110 мм"><NumberInput min={0} className={inp} value={input.funnels110} onChange={(v) => upd("funnels110", v)} /></Field>
              <Field label="d 160 мм"><NumberInput min={0} className={inp} value={input.funnels160} onChange={(v) => upd("funnels160", v)} /></Field>
            </div>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2 mt-4">Аератори, шт</div>
            <div className="grid grid-cols-3 gap-3">
              <Field label="d 75 мм"><NumberInput min={0} className={inp} value={input.aerators75} onChange={(v) => upd("aerators75", v)} /></Field>
              <Field label="d 110 мм"><NumberInput min={0} className={inp} value={input.aerators110} onChange={(v) => upd("aerators110", v)} /></Field>
              <Field label="d 160 мм"><NumberInput min={0} className={inp} value={input.aerators160} onChange={(v) => upd("aerators160", v)} /></Field>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
              <Field label="Точки обпайки, шт" hint="Виходи труб, стійки, нестандартні примикання.">
                <NumberInput min={0} className={inp} value={input.opaikaPoints} onChange={(v) => upd("opaikaPoints", v)} />
              </Field>
              <Field label="ПВХ-уголок, п.м" hint="0 — рахувати автоматично: периметр × запас.">
                <NumberInput min={0} className={inp} value={input.pvcAngleMeters} onChange={(v) => upd("pvcAngleMeters", v)} placeholder={`авто ≈ ${formatNum(input.perimeter * DEFAULT_PVC_COEFFS.angleReserve, 1)}`} />
              </Field>
              <Field label="Прижимна планка, п.м" hint="0 — рахувати автоматично від периметру.">
                <NumberInput min={0} className={inp} value={input.pvcClampMeters} onChange={(v) => upd("pvcClampMeters", v)} placeholder={`авто ≈ ${formatNum(input.perimeter * DEFAULT_PVC_COEFFS.clampReserve, 1)}`} />
              </Field>
              <Field label="Капельник, п.м" hint="0 — дорівнює периметру.">
                <NumberInput min={0} className={inp} value={input.dripEdgeMeters} onChange={(v) => upd("dripEdgeMeters", v)} placeholder={`авто ≈ ${input.perimeter}`} />
              </Field>
            </div>
            <Tip>
              Розхідники (рондоль, дюбель, шуруп, герметик) рахуються автоматично: рондоль {DEFAULT_PVC_COEFFS.rondelPerMeterVert} шт/п.м вертикалі, дюбель+шуруп {DEFAULT_PVC_COEFFS.dowelsPerMeterStrip} шт/п.м планки.
            </Tip>
          </section>
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={input.withPrep} onChange={(e) => upd("withPrep", e.target.checked)} />Підготовка поверхні</label>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={input.withGeotextile} onChange={(e) => upd("withGeotextile", e.target.checked)} />Геотекстиль-розділювач</label>
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {showInternal && (<>
                ...
                <Stat label="Маржа" value={`${formatNum(result.marginPercent, 1)} %`}
