...
export const Route = createFileRoute("/objects/")({
  ...
  head: () => ({ meta: [
    { title: "Об'єкти — TERZI" },
    { name: "description", content: "Реєстр об'єктів TERZI: клієнти, послуги, статуси, менеджери, дати." },
  ]}),
...
function ObjectsPage() {
  ...
  const { data = [], isLoading } = useQuery({ queryKey: ["objects"], queryFn: () => listFn() });
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-[1400px] mx-auto space-y-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight">Об'єкти</h1>
            <p className="text-sm text-muted-foreground mt-1">Центральна сутність: замер → розрахунок → смета → договір → виробництво</p>
          </div>
          <Link to="/objects/new" className="inline-flex items-center gap-2 bg-primary text-primary-foreground rounded-md px-4 py-2 text-sm font-semibold hover:opacity-90">
            <Plus className="w-4 h-4" /> Створити об'єкт
          </Link>
        </div>
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead className="sticky-thead bg-secondary/60 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="text-left px-3 py-2">Об'єкт</th>
              ...
              <tbody>
                ...
                {!isLoading && rows.length === 0 && (
                  <tr><td colSpan={10} className="p-8 text-center text-muted-foreground">
                    ...
                    Немає об'єктів. Створіть перший.
                  </td></tr>
                )}
                {rows.map((r: any) => (
                  <tr key={r.id} className="border-t border-border hover:bg-secondary/30">
                    ...
                    <td className="px-3 py-2 text-right">
                      <Link to="/objects/$id" params={{ id: r.id }} className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
