...
function DemolitionPage() {
    ...
    const row = await saveFn({
      data: {
        ...
        calculation_json: buildEstimateSnapshot({
          ...
          norms: { coefficients: demolitionCoeffs },
          priceSources,
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 relative">
      <CalcStepRail module="demolition" />
      <img
        src={logoAsset.url}
        alt=""
        aria-hidden="true"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Стяжка / плитка / покрівля / перегородки + вивіз
            </p>
          </div>
        </div>
      ...
      {view === "production" && (
        <div className="relative z-10">
          <GenericProductionCard lines={result.lines} title={"Демонтаж"} estimateNumber={estimateNumber} address={client.address} />
        </div>
      )}
          ...
          <section className="panel p-4 md:p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <Field label="Ручний об'єм сміття, м³ (опц.)">
                <NumberInput
                  className={inp}
                  value={input.manualHaulM3 ?? 0}
                  onChange={(v) => upd("manualHaulM3", v || undefined)}
                />
              </Field>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="Розмір контейнера">
                <select
                  ...
                  onChange={(e) => upd("containerSize", +e.target.value as ContainerSize)}
                >
              ...
              <label className="flex items-center gap-2 text-sm mt-6">
                <input
                  type="checkbox"
                  checked={input.withBags}
                  onChange={(e) => upd("withBags", e.target.checked)}
                />
                Винос мішками
              </label>
                ...
                {showInternal && (
                  <>
                    ...
                    <Stat
                      ...
                      highlight={result.marginPercent >= demolitionCoeffs.marginThreshold}
                      warn={result.marginPercent < demolitionCoeffs.marginThreshold}
                    />
                  </>
                )}
              </div>
